...
class RoundRobin(tk.Toplevel):
    ...
    def __init__(self, parent, channel, window):
        ...
        self.labelTop = tk.Label(self.frameTop, text='ROUND ROBIN', font=("Arial", 20, "bold", "italic"), bg='#d2e9fa')
        ...
        self.frameLeft.rowconfigure((0, 1, 2, 3, 4, 5, 6, 7), weight=1)
        ...
        self.listboxWindows.grid(column=2, row=3, padx=5, pady=10, sticky="NSEW")
        self.scrollWindows.grid(column=3, row=3, sticky="NS")
    ...
    def resizeFrameLeft(self, event=0):
        ...
        self.labelFrec.grid(column=1, row=0, padx=int(self.winfo_width()*0.01), pady=int(self.winfo_height()*0.035), sticky="E")
        self.labelCyc.grid(column=1, row=1, padx=int(self.winfo_width()*0.01), pady=int(self.winfo_height()*0.035), sticky="E")
        self.labelAmpli.grid(column=1, row=2, padx=int(self.winfo_width()*0.01), pady=int(self.winfo_height()*0.035), sticky="E")
        self.labelWindowed.grid(column=1, row=3, padx=int(self.winfo_width()*0.01), pady=int(self.winfo_height()*0.035), sticky="E")
        self.labelPhase.grid(column=1, row=4, padx=int(self.winfo_width()*0.01), pady=int(self.winfo_height()*0.035), sticky="E")
        self.labelInitTime.grid(column=1, row=5, padx=int(self.winfo_width()*0.01), pady=int(self.winfo_height()*0.035), sticky="E")
        self.labelSamples.grid(column=1, row=6, padx=int(self.winfo_width()*0.01), pady=int(self.winfo_height()*0.035), sticky="E")
        ...
        if globals.connected:
            ...
            if not self.frec.isdigit():
                ...
                if int(self.frec) <= 0:
                    ...
                    tk.messagebox.showinfo("Amplitude error", "Amplitude must be between 0 and 1", parent=self)
                    self.focus_force()
                        ...
                        if not self.address:
                            ...
                            if not self.name or not correctName:
                                ...
                                with h5py.File(self.addressFile, "a") as f:
                                    try:
                                        grp = f.require_group(self.group)
                                        dset = grp.create_dataset(self.name + "Params", (7,2), dtype='S30')
                                        dset.attrs['Params'] = True

                                        dset[0, 0] = "Frequency (Hz)"
                                        dset[1, 0] = "Cycles"
                                        dset[2, 0] = "Amplitude"
                                        dset[3, 0] = "Windowed"
                                        dset[4, 0] = "Phase"
                                        dset[5, 0] = "Acquisition start (us)"
                                        dset[6, 0] = "Acquisition time (samples)"
                                        ...
                                        dset[1, 1] = self.cyc
                                        dset[2, 1] = self.ampl
                                        dset[3, 1] = self.valueW
                                        dset[4, 1] = self.phase
                                        dset[5, 1] = self.initTime
                                        dset[6, 1] = self.sampl
    ...
    def progressBarFunction(self):
        ...
        progressBar = ProgressBar.ProgressBarClass(self, self.addressFile, self.name, self.group, "RoundRobin")
